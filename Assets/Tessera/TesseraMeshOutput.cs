...
namespace Tessera
{
    ...
    public class TesseraMeshOutput : MonoBehaviour, ITesseraTileOutput
    {
        ...
        private CombineInstance? GetCombineInstance(TesseraTileInstance i, GameObject subObject, Matrix4x4 transform, object material)
        {
            ...
            {
                // Make new mesh
                var newMesh = i.MeshDeformation.Transform(mesh, subMeshIndex);
        ...
        private CombineInstance? GetCombineInstance(TesseraTileInstance i, object material)
        {
            var combineInstances = new List<CombineInstance>();
            void HandleSubObject(GameObject subObject, Matrix4x4 transform)
            {
                var ci = GetCombineInstance(i, subObject, transform, material);
            ...
            {
                return combineInstances[0];
            }
            // TODO: 
            throw new NotImplementedException("instantiateChildrenOnly not supported with mesh output");
        }

        public void UpdateTiles(IEnumerable<TesseraTileInstance> tileInstances)
        {
            var allMaterialsList = GetMaterialsList(tileInstances);
            ...
            foreach (var material in allMaterialsList)
            {
                ...
                {
                    //if(kv.Value.TryGetValue(material, out var ci))
                    if (GetCombineInstance(kv.Value, material) is CombineInstance ci)
                    {
                        if (ci.mesh == null)
                        {
                            //Debug.Log((kv.Key, kv.Value, material, ci));
                        }
                        else
                        {
