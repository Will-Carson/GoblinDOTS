...
namespace Tessera
{
    ...
    public abstract class TesseraTileEditorToolBase : EditorTool
    {
        ...
        bool IsHighlight(Hit hit, TesseraTileBase t, Vector3Int offset, FaceDir dir, SubFace p)
        {
            if (hit?.target != t)
                return false;
        ...
        private void Repaint(Hit hit, int controlId)
        {
            ...
            var drawnWireframes = new HashSet<(TesseraTileBase, Vector3Int, FaceDir)>();
            ...
            foreach (var ((h, faceDetails, index), doWireframe) in facesToDraw.Zip(doWireframes, (a, b) => Tuple.Create(a, b)))
            {
                ...
                var vertices = GetCellDrawingType(t).GetSubFaceVertices(offset, dir, p, t.center, t.tileSize)
                ...
                if (doWireframe)
                {
                    vertices = GetCellDrawingType(t).GetFaceVertices(offset, dir, t.center, t.tileSize)
                    .Select(t.transform.TransformPoint)
                    .ToArray();
        ...
        public override void OnToolGUI(EditorWindow window)
        {
            ...
            if (pencilHit != null)
            {
                ...
                var faceDetails = pencilHit.target.Get(pencilHit.offset, pencilHit.dir);
                ...
                else
                {
                    ...
                    foreach (var (offset, dir, faceDetails) in t.faceDetails)
                    {
                        foreach (var p in cellDrawingType.GetSubFaces(offset, dir))
                        {
                            ...
                            {
                                cellDrawingType.SetSubFaceValue(faceDetails, p, TesseraTilePaintingState.paintIndex);
                            }
                        }
                    }
                }
