...
float sdf_noise(float3 p, float3 boundsMin, float3 boundsMax, float3 offset, float3 size, float threshold, int numOctaves, float octaveOffsetFactor, float3 period = 100.0f)
{
  //float d = threshold - (0.8f * (saturate(pnoise(p / size, offset, numOctaves, octaveOffsetFactor, period)) - 0.5f) + 0.5f);
  float d = threshold - cached_noise(p / size, offset, numOctaves, octaveOffsetFactor);
  ...
  d *= 0.5f * min(min(size.x, size.y), size.z);
