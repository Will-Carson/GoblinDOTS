...
namespace Tessera
{
    public class CubeCellDrawingType : ICellDrawingType
    {
        ...
        public Vector3[] GetSubFaceVertices(Vector3Int offset, FaceDir dir, SubFace subface, Vector3 center, Vector3 tileSize)
        {
        ...
        public Vector3[] GetFaceVertices(Vector3Int offset, FaceDir dir, Vector3 center, Vector3 tileSize)
        {
            var u = new DrawingUtil(center, tileSize);
        ...
        public void GetFaceCenterAndNormal(Vector3Int offset, FaceDir dir, Vector3 center, Vector3 tileSize, out Vector3 faceCenter, out Vector3 faceNormal)
        {
            var u = new DrawingUtil(center, tileSize);
        ...
        public RaycastCellHit Raycast(Ray ray, Vector3Int offset, Vector3 center, Vector3 tileSize, float? minDistance, float? maxDistance)
        {
            ...
            var currentHit = Raycast(center + u.ScaleBySize(offset), u.hsize, ray, out var currentHitDir, out var currentHitPoint);
        ...
        public SubFace RoundSubFace(Vector3Int offset, FaceDir dir, Vector2 p2, PaintMode paintMode)
        {
            return SquareFaceDrawingUtils.RoundSubFace(p2, paintMode);
        }

        public bool IsAffected(Vector3Int parentOffset, FaceDir parentFaceDir, SubFace parentSubface, Vector3Int childOffset, FaceDir childFaceDir, SubFace childSubface, PaintMode paintMode)
        {
            var p2 = SquareFaceDrawingUtils.ToSubFaceVector(parentSubface);
            var p1 = SquareFaceDrawingUtils.ToSubFaceVector(childSubface);


            var up = childFaceDir.Up();
            var forward = childFaceDir.Forward();
            var right = Vector3.Cross(forward, up);


            var up2 = parentFaceDir.Up();
            var forward2 = parentFaceDir.Forward();
        ...
        public IEnumerable<SubFace> GetSubFaces(Vector3Int offset, FaceDir dir)
        {
        ...
        public Vector3Int Move(Vector3Int offset, FaceDir dir)
        {
            return offset + dir.Forward();
        }
