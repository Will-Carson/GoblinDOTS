...
SdfBrushMaterial lerp(SdfBrushMaterial a, SdfBrushMaterial b, float t)
{
  ...
  o.emission = lerp(a.emission, b.emission, t);
...
float sdf_brush(float res, inout float3 p, SdfBrush b)
{
  ...
  switch (b.type)
  {
    ...
    case kSdfNoiseVolume:
    {
      ...
      {
        float n = sdf_noise(aSample[i], -h, h, offset, size, threshold, numOctaves, octaveOffsetFactor, aPeriod[i]);
    ...
    case kSdfNoiseCurveSimple:
    {
      ...
      if (useNoise)
      {
        ...
        int numOctaves = int(b.data1.w);
        float octaveOffsetFactor = b.data2.x;
        float n = sdf_noise(s, -kInfinity, kInfinity, offset, size, threshold, numOctaves, octaveOffsetFactor, kCartesianNoisePeriod);
        res = sdf_int_smooth(res, n, 0.5f * r);
      }
      break;
    }

    case kSdfNoiseCurveFull:
    {
      ...
      if (numPoints > 1)
      {
        ...
        for (int i = 1, n = numPoints - 2; i < n; ++i, ++iA)
        {
          ...
          for (float t = dt; t < 1.0001f; t += dt)
          {
            ...
            float d = sdf_round_cone(pRel, prevPos, currPos, r, r + dr);
            if (d < res)
            {
              res = d;
              float2 segRes = sdf_segment(pRel, prevPos, currPos);
              iClosest = i;
              tClosest = t;
            ...
            && useNoise)
        {
          ...
          float3 pC = aBrush[iA + 2].data0.xyz;
          float3 pD = aBrush[iA + 3].data0.xyz;
          float3 segPosA = catmull_rom(pA, pB, pC, pD, tClosest - dt);
          float3 segPosB = catmull_rom(pA, pB, pC, pD, tClosest);
          float3 up = normalize(kUnitY + 1e-3f * rand(aBrush[iA].data0.xyz));
          float3 front = normalize(segPosB - segPosA);
          float3 left = normalize(cross(up, front));
          up = cross(front, left);
          float3 s = float3(closestLen, dot(pRel - pClosest, up), dot(pRel - pClosest, left));
...
float sdf_brush_normal(float res, inout float3 p, float3 pInvariantOffset, SdfBrush b)
{
  float d = sdf_distortion_modifier_bounds_query(p, b);
  float3 oldP = p;
  p += pInvariantOffset;
  res = sdf_brush(res, p, b);
  if (d > 0.0f)
  {
    p = oldP;
  }
...
float sdf_brush_apply(float res, inout float3 p, SdfBrush b)
{
...
float sdf_brush_normal_apply(float res, inout float3 p, float3 pInvariantOffset, SdfBrush b)
{
  float d = sdf_brush_normal(res, p, pInvariantOffset, b);
...
float sdf_brush_apply(float res, inout float3 p, SdfBrush b, inout SdfBrushMaterial oMat)
{
  float d = sdf_brush(res, p, b);
  ...
  switch (b.op)
  {
    ...
    case kSdfSubtract:
      res = sdf_sub_smooth(res, d, blend);
      tMat = 1.0f - saturate(2.0f * (d - voxelSize) / blend);
      break;
    ...
    case kSdfDye:
      tMat = 1.0f - saturate(max(0.0f, d) / blend);
      break;
      ...
      && b.materialIndex >= 0)
  {
    ...
    oMat = lerp(oMat, aBrushMaterial[b.materialIndex], tMat);
  }

  return res;
}

float sdf_all_brushes(float3 p, int iBrushMask)
{
  float res = kInfinity;
  for (int i = 0; i < numBrushes; ++i)
    res = sdf_brush_apply(res, p, aBrush[i]);

  return res;
}

float sdf_all_brushes_normal(float3 p, float3 pInvariantOffset, int iBrushMask)
{
  float res = kInfinity;

  for (int i = 0; i < numBrushes; ++i)
    res = sdf_brush_normal_apply(res, p, pInvariantOffset, aBrush[i]);
...
float sdf_all_brushes(float3 p, int iBrushMask, out SdfBrushMaterial mat)
{
  ...
  float res = kInfinity;
  for (int i = 0; i < numBrushes; ++i)
    res = sdf_brush_apply(res, p, aBrush[i], mat);

  return res;
}

float sdf_masked_brushes(float3 p, int iBrushMask)
{
  float res = kInfinity;
  //if (iBrushMask >= 0)
  {
    FOR_EACH_BRUSH(iBrushMask, 
      res = sdf_brush_apply(res, p, aBrush[iBrush]);
    );
  }
  /*
  else
  {
    res = sdf_all_brushes(p, iBrushMask);
  }
  */
...
float sdf_masked_brushes_normal(float3 p, float3 pInvariantOffset, int iBrushMask)
{
  float res = kInfinity;
  //if (iBrushMask >= 0)
  {
    FOR_EACH_BRUSH(iBrushMask, 
      res = sdf_brush_normal_apply(res, p, pInvariantOffset, aBrush[iBrush]);
    );
  }
  /*
  else
  {
    res = sdf_all_brushes_normal(p, pInvariantOffset, iBrushMask);
  }
  */
...
float sdf_masked_brushes(float3 p, int iBrushMask, out SdfBrushMaterial mat)
{
  ...
  {
    FOR_EACH_BRUSH(iBrushMask, 
      res = sdf_brush_apply(res, p, aBrush[iBrush], mat);
    );
  }
  /*
  else
  {
    res = sdf_all_brushes(p, iBrushMask, mat);
  }
  */
